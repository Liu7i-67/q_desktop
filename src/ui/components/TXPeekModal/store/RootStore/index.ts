...
import { ITXPeekModalProps } from "../../interface";
...
export class RootStore implements IRootStore {
  ...
  constructor(refs: IRefs) {
    this.refs = refs;
    this.propsStore = new PropsStore<ITXPeekModalProps>();
...
export const { Provider, useStore } = createContainer(() => {
  const collaborationModalRef = useRef<ICollaborationModalRef>(null);
  return useLocalObservable(
    () =>
      new RootStore({
        collaborationModalRef,
      })
  );
});
