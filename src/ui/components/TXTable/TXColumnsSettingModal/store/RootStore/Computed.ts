...
export class Computed implements IComputed {
  ...
  get indeterminate() {
    ...
    if (logic.leftList.length) {
      return true;
    }
    if (logic.rightList.length) {
      return true;
    }
    return false;
  }
  ...
  get checkedAll() {
    const { logic } = this.rootStore;
    return (
      logic.columnMap.size ===
      logic.renderList.length + logic.leftList.length + logic.rightList.length
    );
  }
  ...
  get allKey() {
    ...
    return (
      logic.initData?.columns?.filter((c) => {
        if (c.filterTitle) {
          return c.filterTitle.includes(logic.searchVal);
        }
        if (typeof c.title === "string") {
          return c.title.includes(logic.searchVal);
        }
        return false;
      }) || []
    );
