...
import { save_channel_type, update_channel_type } from "../../service";
...
export class Logic implements ILogic {
  ...
  async onSave() {
    ...
    const [err, res] = await to(save_channel_type(values));
    runInAction(() => {
      if (!(err || !res)) {
  ...
  async onEdit() {
    ...
    const [err, res] = await to(
      update_channel_type({
        ...
        id: (this.initData.node as TNode).key,
      })
    );
    runInAction(() => {
      if (!(err || !res)) {
  ...
  initFormByCreate() {
    ...
    if (this.initData.node) {
      refs.form.setFieldsValue({
        parentId: this.initData.node.key,
      });
    }
  }
  ...
  initFormByEdit() {
    ...
    if (this.initData.node) {
      refs.form.setFieldsValue({
        ...
        typeName: this.initData.node.title,
