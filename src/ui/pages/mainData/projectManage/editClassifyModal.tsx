...
const EditClassifyModal = () => {
  ...
  const { runSaveProjectType, runUpdateProjectType } = useSelector(
    (x) => x.api
  );
  const {
  ...
  return (
    <Modal
      ...
      onOk={isCreateClassify ? saveProjectType : updateProjectType}
      destroyOnClose
      width={500}
      confirmLoading={
        isCreateClassify
          ? runSaveProjectType.loading
          : runUpdateProjectType.loading
      }
    >
      <Form
