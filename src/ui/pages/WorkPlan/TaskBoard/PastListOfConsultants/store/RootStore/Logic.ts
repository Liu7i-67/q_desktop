...
export class Logic implements ILogic {
  ...
  onAction(action: TAction, record: IPastListOfConsultants) {
    const { propsStore } = this.rootStore;
    switch (action) {
      case "DETAIL":
        {
          ...
          window.open(
            `/#/workplan/taskOverview?comDateStart=${start}&&comDateEnd=${end}&&type=taskList&&taskOrg=${record?.createBy || ""}&&taskId=${propsStore.props.taskId || ""}`
          );
        }
        break;
    }
