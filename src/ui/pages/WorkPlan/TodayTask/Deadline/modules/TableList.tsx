...
export const TableList = observer(function TableList_() {
  ...
  const columns: ITXColumnType<IDeadline>[] = [
    {
      width: 12,
      key: "index",
  ...
  return (
    <TXTable<IDeadline>
      ...
      rowClassName={(record: IDeadline, index: number) => {
        if (record.priorityFlag === true) {
          return "!bg-[#FEF6F0]";
        }
        if (record.lastForgetFlag === true) {
      ...
      tableKey="TODAY_DEADLINE_UNFINISHED"
      size="small"
      columns={columns}
      ...
      locale={{
        emptyText: (
          <NoData
            title="你已经完成所有今日截止任务"
            description="快去看看其他今日待办任务吧～"
            linkText="查看待办任务"
            onLinkClick={() => {
              propsStore.props?.changeTab?.();
            }}
          />
        ),
      }}
